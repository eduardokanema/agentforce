...
describe("PlanModePage", () => {
  ...
  it("drives the compose, generate, edit, and launch flow", async () => {
    ...
    const fetchMock = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === "/api/config") {
          return new Response(
            JSON.stringify({
              filesystem: { allowed_base_paths: ["/Users/eduardo/Projects"] },
            }),
            {
              headers: { "Content-Type": "application/json" },
            },
        ...
        if (url.startsWith("/api/filesystem")) {
          return new Response(
            JSON.stringify({
              path: "/Users/eduardo/Projects",
              entries: [
                {
                  name: "agentforce",
                  path: "/Users/eduardo/Projects/agentforce",
                  is_dir: true,
                },
              ],
              parent: null,
    ...
    const selectFolderButton = Array.from(
      container.querySelectorAll("button"),
    ).find((button) => button.textContent?.includes("Select this folder")) as
      | HTMLButtonElement
      | undefined;
    if (selectFolderButton && !selectFolderButton.disabled) {
      await act(async () => {
        selectFolderButton.dispatchEvent(
    ...
    const missionName = container.querySelector(
      'input[aria-label="Mission name"]',
    ) as HTMLInputElement;
    expect(missionName.value).toBe("Launch Mission");

    const taskSelect = container.querySelector(
      'select[aria-label="Task model"]',
    ) as HTMLSelectElement;
    const optionLabels = Array.from(taskSelect.options).map(
      (option) => option.value,
    );
    ...
    const rawYaml = container.querySelector(
      'textarea[aria-label="Raw YAML"]',
    ) as HTMLTextAreaElement;
    expect(rawYaml.value).toContain("Launch Mission");

    await act(async () => {
      rawYaml.value = rawYaml.value.replace(
        "Launch Mission",
        "Launch Mission Edited",
      );
      rawYaml.dispatchEvent(new Event("input", { bubbles: true }));
    });

    const launchButton = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.includes("Launch Mission"),
    ) as HTMLButtonElement;
