...
type MissionBadgeStatus = 'active' | 'complete' | 'failed' | 'needs_human' | 'in_progress';
...
function getTaskState(taskState: TaskState | undefined, taskSpec: { id: string }, startedAt: string): TaskState {
  return (
    taskState ?? {
      task_id: taskSpec.id,
      status: 'pending',
...
export function filterEventLogEntries(entries: EventLogEntry[], filterText: string): EventLogEntry[] {
  const filter = filterText.trim().toLowerCase();
  if (!filter) {
    return entries;
  }

  return entries.filter((entry) => entry.event_type.toLowerCase().includes(filter));
}
...
function MissionDetailPageContent({ missionId }: { missionId: string }) {
  ...
  const avgReviewScore = avgScores.length > 0
    ...
    : '—';
  const taskIndexById = new Map(mission.spec.tasks.map((task, index) => [task.id, index + 1]));
  const missionStatus = getMissionStatus(mission.task_states);
  const workspacePath = mission.working_dir?.trim() || mission.spec.working_dir?.trim() || '—';
    ...
    <div>
      ...
      <div className="mb-6 flex flex-col gap-3">
        <div className="flex flex-wrap items-start gap-3">
          ...
          <StatusBadge
            ...
            className={missionStatus === 'in_progress' ? 'animate-[pulse-glow_2s_ease_infinite]' : ''}
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 text-[11px] text-dim">
          ...
          <span className="text-[10px] uppercase tracking-[0.08em] text-muted">Workspace path</span>
          <span className="max-w-[26rem] truncate rounded-full border border-border bg-surface px-3 py-0.5 font-mono text-[11px] text-muted" title={workspacePath}>
            {workspacePath}
          </span>
        </div>
          ...
          <button
            ...
            onClick={() => {
              setPendingAction({
                ...
                message: 'This will launch a fresh run from the current mission state.',
      ...
      <section className="sec">
        ...
        <div className="grid grid-cols-1 gap-2 xl:grid-cols-2">
          {mission.spec.tasks.map((taskSpec) => {
            const taskState = getTaskState(mission.task_states[taskSpec.id], taskSpec, mission.started_at);
            const score = taskState.review_score;
            const dependencyLabels = taskSpec.dependencies.map((dependencyId) => getDependencyLabel(dependencyId, taskIndexById));
                ...
                <div className="relative flex items-start justify-between gap-3">
                  <div className="min-w-0 flex items-start gap-3">
                    <span className="inline-flex shrink-0 rounded-full border border-border bg-card px-2 py-0.5 font-mono text-[11px] text-dim">
                      {String(taskIndexById.get(taskSpec.id) ?? 0).padStart(2, '0')}
                    </span>
                    <div className="min-w-0">
                      <div className="text-[13px] font-medium text-text">{taskSpec.title}</div>
                      <p className="mt-0.5 truncate text-[11px] text-dim">{taskSpec.description}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5">
                    ...
                    {score > 0 ? (
                      <span className={`inline-flex rounded border px-2 py-0.5 text-[11px] font-bold ${getScoreClassName(score)}`}>
                        {formatScore(score)}
                      </span>
                    ) : null}
                  </div>
      ...
      <section className="sec">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <label className="text-[11px] uppercase tracking-[0.08em] text-muted" htmlFor="mission-event-filter">
            Event filter
          </label>
          <input
