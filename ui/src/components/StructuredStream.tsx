...
function formatEventLabel(event: StreamEventRecord): string {
  ...
  if (event.kind === "raw_line") {
    const providerEventType = stringValue(
      (event.payload.meta as Record<string, unknown> | undefined)
        ?.provider_event_type,
    );
  ...
  if (event.kind === "usage") {
    const tokensIn =
      typeof event.payload.tokens_in === "number"
        ? event.payload.tokens_in.toLocaleString()
        : "0";
    const tokensOut =
      typeof event.payload.tokens_out === "number"
        ? event.payload.tokens_out.toLocaleString()
        : "0";
    const cost =
      typeof event.payload.cost_usd === "number"
        ? `$${event.payload.cost_usd.toFixed(4)}`
        : "$0.0000";
    return `↓ ${tokensIn} in\n↑ ${tokensOut} out\n${cost}`;
  }
  if (event.kind === "raw_line") {
    if (providerEventType === "thread.started") {
      const threadId = stringValue(
        (event.payload.meta as Record<string, unknown> | undefined)?.thread_id,
      );
  ...
  const blocks = useMemo<Block[]>(() => {
    ...
    for (const event of events) {
      ...
      if (event.kind === "tool_start") {
        const callId =
          stringValue(event.payload.call_id) || `tool-${event.seq}`;
        ...
        <button
          ...
          className={[
            ...
            autoScroll
              ...
              : "border-border text-dim hover:bg-surface",
          ].join(" ")}
        ...
        <div className="space-y-3">
          ...
          {blocks.map((block) => {
            if (block.type === "response") {
              const isLive =
                !done && block.endSeq === events[events.length - 1]?.seq;
              return (
                <article
                  key={block.key}
                ...
                >
                  <div className="mb-2 flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isLive ? (
                        <div className="rounded-full border border-cyan/30 bg-cyan/10 px-2 py-0.5 text-[10px] text-cyan">
                          live
                        </div>
                      ) : null}
                    </div>
                  </div>
                  <pre className="whitespace-pre-wrap break-words font-mono text-[12px] leading-6 text-text">
                    {block.text}
                  </pre>
                </article>
              );
                ...
                {block.kind === "usage" ? (
                  <div className="mt-3 grid gap-2 sm:grid-cols-3">
                    {block.message.split("\n").map((part) => (
                      <div
                        key={part}
                        className="rounded border border-current/10 bg-black/10 px-3 py-2 font-mono text-[12px] text-text"
                      >
                        {part}
                      </div>
                    ))}
                  ...
                  <pre className="mt-2 whitespace-pre-wrap break-words font-mono text-[12px] leading-6 text-text">
                    {block.message}
                  </pre>
                )}
              </article>
